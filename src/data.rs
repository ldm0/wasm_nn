...
impl Data {
    ...
    pub fn init(&mut self, num_classes: u32, num_samples: u32, radius: f32, span: f32, rand_max: f32) {
        ...
        for i in 0..num_classes {
            ...
            let xs = (theta.mapv(f32::sin) * &rho).into_shape((num_samples, 1)).unwrap();
            let ys = (theta.mapv(f32::cos) * &rho).into_shape((num_samples, 1)).unwrap();
            let mut class_points = self.points.slice_mut(s![i * num_samples..(i + 1) * num_samples, ..]);
            class_points.assign(&stack![Axis(1), xs, ys]);
            let mut class_labels = self.labels.slice_mut(s![i * num_samples..(i + 1) * num_samples]);
